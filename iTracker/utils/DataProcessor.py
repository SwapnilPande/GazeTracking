...
class DataPreProcessor(Sequence):
        ...
        def indexData(self, path, loadAllData):
                ...
                for subject in pbar(subjectDirs):
                        ...
                        screen = self.getScreenJSON(subjectPath)

                        #Iterate over frames for the current subject
                        for i, (frame, fv, lv, rv, fgv, ort) in enumerate(zip(frameNames,
                                                                face['IsValid'],
                                                                leftEye['IsValid'],
                                                                rightEye['IsValid'],
                                                                faceGrid['IsValid'],
        ...
        # getScreenJSON
        # Loads screen.json to a dictionary
        # this file contains orientations for the frames in the directory
        # Arguments:
        # subjectPath - Path to unzipped root directory of the subject
        # Files that are read in:
        # - screen.json
        ...
        #
        def getFaceGridsNew(self,imagePaths):
                faceGridSize = 12
                faceGrids = np.zeros((len(imagePaths), faceGridSize))
                for frameNum, frame in enumerate(imagePaths):
                        imageX=int(self.getImage(frame).shape[0])
                        imageY=int(self.getImage(frame).shape[1])
                        #Retrieve necessary values
                        faceGrid = np.zeros(faceGridSize)
                        
                        faceGrid[0]=xFace =float( int(self.metadata[frame]['face']['X'])/imageX)
                        faceGrid[1]=yFace =float( int(self.metadata[frame]['face']['Y'])/imageY)
                        faceGrid[2]=wFace =float( int(self.metadata[frame]['face']['W'])/imageX)
                        faceGrid[3]=hFace =float( int(self.metadata[frame]['face']['H'])/imageY)
                        ...
                        #Therefore, we must transform to make coordinates
                        #Relative to picture by adding coordinates of face
                        faceGrid[4]=xLeft = float(int(self.metadata[frame]['leftEye']['X'])/imageX) + xFace
                        faceGrid[5]=yLeft = float(int(self.metadata[frame]['leftEye']['Y'])/imageY) + yFace
                        faceGrid[6]=wLeft = float(int(self.metadata[frame]['leftEye']['W'])/imageX)
                        faceGrid[7]=hLeft = float(int(self.metadata[frame]['leftEye']['H'])/imageY)

                        #Right Eye
                        faceGrid[8]=xRight = float(int(self.metadata[frame]['rightEye']['X'])/imageX) + xFace
                        faceGrid[9]=yRight = float(int(self.metadata[frame]['rightEye']['Y'])/imageY) + yFace
                        faceGrid[10]=wRight = float( int(self.metadata[frame]['rightEye']['W'])/imageX)
                        faceGrid[11]=hRight = float(int(self.metadata[frame]['rightEye']['H'])/imageY)
        ...
        def __getitem__(self, index):
                ...
                faceGridBatch = self.getFaceGridsNew(framesToRetrieve)
